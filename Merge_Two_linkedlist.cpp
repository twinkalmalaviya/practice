...
public:
	...
	/* To  merge stored linked list  */
node* MergelinkedList_Sorted_linkedList( node* head_One,  node* head_Two)
{
 if (!head_One) 
    {
        return head_Two;    
...
int main() {
	...
	c.add_node(9, 3);
	a.display(c.gethead());
	a.update_head(b.MergelinkedList_Sorted_linkedList(a.gethead(),c.gethead()));
  a.display(a.gethead());
  a.display(a.gethead());
  a.display(c.gethead());
